...
void* child(void* params) {
	...
	args->squaredId = childID*childID; //Square the thread id
...
int main(int argc, char** argv) {
	...
	for (unsigned int id = 0; id < numThreads; id++) {
		pthread_join(children[id], NULL ); //Wait for all threads to finish
	}
	for (unsigned int id = 0; id < numThreads; id++)
		printf("SquaredId of #%d: %d\n", id, args[id].squaredId); //Print squaredId of all threads
